...
public class XLuaManager : MonoSingleton<XLuaManager>
{
    ...
    private readonly string EmmyLuaDebugScriptName = "EmmyLuaDebugger";
...
#if UNITY_EDITOR
            LoadScript(EmmyLuaDebugScriptName);
#endif
            //注册pblua
            luaEnv.AddBuildin("pb", XLua.LuaDLL.Lua_Third.LoadPb);
            //注册rapidjson
            luaEnv.AddBuildin("rapidjson", XLua.LuaDLL.Lua_Third.LoadRapidJson);
        }
        else
        {
    ...
    public object[] SafeDoStringReturn(string scriptContent)
    {
        if (luaEnv != null)
        {
            try
