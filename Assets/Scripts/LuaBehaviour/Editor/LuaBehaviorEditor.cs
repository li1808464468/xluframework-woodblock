...
public class LuaBehaviourEditor : Editor
{
    ...
    public DefaultAsset LuaScript
    {
        ...
        get
        {
            return AssetDatabase.LoadAssetAtPath(LuaPath + m_LuaScriptPath.stringValue + ".lua", typeof(DefaultAsset)) as DefaultAsset;
        }
        set
        {
            ...
            if((!string.IsNullOrEmpty(path)|| value == null) && m_LuaScriptPath.stringValue != path)
            {
    ...
    {
        var scriptPath = LuaPath + m_LuaScriptPath.stringValue + ".lua";
        if (File.Exists(scriptPath))
        {
            long curTime = File.GetLastWriteTime(scriptPath).Ticks;
    ...
    public const string LuaPath = "Assets/LuaScripts/";
    ...
    {      
        ...
        {
            var rets = XLuaManager.Instance.SafeDoStringReturn($"return require \"{m_LuaScriptPath.stringValue}\"");
            luaClass = (LuaTable)rets[0];
        }
